...
export default function ProfileScreen() {
  ...
  const handleLogout = () => { closeMenu(); router.replace('/login'); };
  const handleEdit = () => { closeMenu(); router.push('/profile/edit'); };
  const handleSettings = () => { closeMenu(); router.push('/profile/settings'); };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <Menu
          visible={menuVisible}
          onDismiss={closeMenu}
          anchor={<IconButton icon="cog-outline" size={24} onPress={openMenu} />}>
...
const styles = StyleSheet.create({
  ...
  header: { position: 'absolute', top: 50, right: 10 },
  name: { marginTop: 16, fontWeight: 'bold' },
  detailsContainer: { flexDirection: 'row', marginTop: 8, alignItems: 'center' },
  detailText: { color: 'gray' },
  detailSeparator: { marginHorizontal: 8, color: 'gray' },
  interestTitle: { marginTop: 40, marginBottom: 16 },
  chipsContainer: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', paddingHorizontal: 20 },
  chip: { margin: 4 },
});