...
// --- THE REDUCER FUNCTION THAT APPLIES THE ACTIONS ---
const eventsReducer = (state: State, action: Action): State => {
  switch (action.type) {
    ...
    case 'TOGGLE_REGISTRATION':
        ...
        if (newRegisteredIds.has(action.payload)) {
            newRegisteredIds.delete(action.payload);
        } else {
            newRegisteredIds.add(action.payload);
        }
    // ... other cases (UPDATE, DELETE, etc.) are the same
    case 'TOGGLE_SAVE':
        const newSavedIds = new Set(state.savedEventIds);
        if (newSavedIds.has(action.payload)) {
            newSavedIds.delete(action.payload);
        } else {
...
export const EventsProvider = ({ children }: { children: ReactNode }) => {
  const initialState: State = {
    events: [
        { id: '1', title: 'Introduction to Python', description: 'A great starting point for coding.', imageUrls: ['https://picsum.photos/700'], date: new Date('2025-10-03T14:00:00'), location: 'Main Auditorium', clubName: 'TinkerHub CET', clubLogoUrl: 'https://picsum.photos/100' },
        { id: '2', title: 'MAKE-A-TON 5.0', description: 'Build something amazing in 48 hours.', imageUrls: ['https://i.imgur.com/SFe4TEG.png', 'https://picsum.photos/702'], date: new Date('2025-10-04T09:00:00'), location: 'Online', clubName: 'IEEE Club', clubLogoUrl: 'https://picsum.photos/101' },
    ],
    registeredEventIds: new Set(),
    savedEventIds: new Set(),
    followedClubNames: new Set(['TinkerHub CET']), // Start by following one club
    filter: 'all',
  };
